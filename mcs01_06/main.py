...
def select_12(session: Session, group_id: int, subject_id: int):
    subq = (select(func.max(Grade.date_received).label("max_date"))
            .join(Student).where(Grade.subject_id == subject_id, Student.group_id == group_id).scalar_subquery())
    stmt = (
        select(Student.full_name, Grade.grade, Grade.date_received)
        .join(Grade, Student.id == Grade.student_id)
        .where(Grade.subject_id == subject_id, Student.group_id == group_id, Grade.date_received == subq)
    )
    return session.execute(stmt).all()
...
if __name__ == "__main__":
    ...
    result_12 = select_12(session, 2, 1)
    ...
    print("Оцінки студентів у певній групі з певного предмета на останньому занятті:", result_12)
